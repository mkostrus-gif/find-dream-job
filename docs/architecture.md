...
Версия схемы явно хранится в `PRAGMA user_version`; схема v11 обновляет
поддерживаемые версии v1–v10. Она сохраняет P0/P1/P2 и добавляет append-only
доказательства попыток и решений восстановления HH. Схема v10 сохраняет P0/P1
и добавляет контур управления получением HH. Схема v9 сохранила контракт
проекций и `lease` v8 и добавила
...
## Долговечный автомат состояний ежедневного запуска (схема v9, сохранён в v11)
...
## Контур получения HH P2 (схемы v10–v11)
...
Схема v11 добавляет `hh_transient_error_attempts`,
`hh_capture_audit_decisions`, `hh_stream_recovery_resolutions` и отдельные
`hh_session_rollover_pages`. Поэтому отклонённый снимок новой сессии остаётся
проверяемым audit-only артефактом, но не участвует в покрытии. Решение точного
недоступного хвоста неизменяемо и идемпотентно; другое последующее решение
считается конфликтом. Полный rollover сначала накапливает непрерывную новую
сессию от страницы 0 в отдельной таблице и только после проверки целиком
заменяет активное доказательство, не смешивая страницы.
