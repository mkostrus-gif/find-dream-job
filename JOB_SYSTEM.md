...
Стандартный путь схем v10–v11 использует проверяемый DOM-адаптер только для чтения
...
python3 scripts/jobctl.py plan-hh-acquisition \
  --run-id <run_id> --stream-key <stream_key> --source-kind ordinary_search \
  --query-json tmp/<stream>-query.json \
  --defer-render --run-lease <token> --json
python3 scripts/jobctl.py validate-hh-capture \
  --capture tmp/<stream>-page-0000.json --json
python3 scripts/jobctl.py record-hh-page \
  --run-id <run_id> --stream-key <stream_key> \
  --capture tmp/<stream>-page-0000.json \
  --defer-render --run-lease <token> --json
python3 scripts/jobctl.py next-hh-work \
  --run-id <run_id> --stream-key <stream_key> --json
```

Для ограниченной очереди подробностей применяется `record-hh-detail`; обычный поток
закрывается через `finalize-hh-stream`, а личные рекомендации — только через
`finalize-hh-personal-recommendations`. Состояние и неизменяемую историю
показывает `inspect-hh-checkpoint`. Небезопасный курсор отключается явной
командой `invalidate-hh-checkpoint --reason ...`. Все команды записи активного
запуска требуют точные `run_id`, `lease`, `--defer-render` и блокировку записи
P0; одинаковый хеш снимка обрабатывается идемпотентно.

Повторяющаяся видимая 502 на точной следующей хвостовой странице не является
общим разрешением пропускать ошибки. После явного включения узкой политики
каждая независимая попытка записывается командой
`record-hh-transient-error`; `inspect-hh-checkpoint` показывает попытки,
audit-only снимки и решение. После не менее пяти попыток точное решение
записывается так:

```bash
python3 scripts/jobctl.py resolve-hh-recovery \
  --run-id <run_id> --stream-key <stream_key> \
  --strategy accepted_unavailable_tail \
  --reason "повторяющаяся видимая 502 на точном хвосте" \
  --defer-render --run-lease <token> --json
```

Если новую сессию можно переснять полностью, каждую её страницу от 0 записывают
через `record-hh-rollover-page`, затем используют стратегию
`full_session_rollover`. Частичная новая сессия не разрешает завершение.
...
URL вакансий, а затем документированные резервные семантические селекторы. Он не
нажимает отклик, сообщение, вступление, архивирование или другой изменяющий
элемент. Страница входа, CAPTCHA, отказ в доступе, активный индикатор загрузки,
отсутствующий числовой ID HH, противоречивая пагинация или изменившиеся данные сессии
...
свежей успешной контрольной точки того же потока, запроса, конфигурации и версии
адаптера, без аномалии, с валидной предыдущей границей и достаточным числом
чистых запусков `shadow`. Обычная граница известных результатов требует минимального перекрытия,
двух последовательных полностью известных и неизменившихся стабильных страниц,
пересечения с прежним покрытием, совместимого порядка и отдельной защитной страницы.
Если доказательство не складывается, P2 продолжает полный просмотр; предел
безопасности без исчерпания источника или доказанной границы блокирует завершение.

В `shadow` вычисленная граница не останавливает просмотр. Оставшаяся выдача
сравнивается программно, и только настроенное число чистых запусков допускает
поток к `delta`. После активации выполняется периодический полный аудит.
Найденный за предсказанной границей новый или материально изменившийся ID
создаёт `incremental_safety_failure`, инвалидирует контрольную точку и возвращает
поток в `shadow`/`full`.

Сверка страницы выполняется пакетным SQL по `vacancies` и
`vacancy_external_aliases`. Результаты — `known_unchanged`, `known_changed`,
`new`, `duplicate_on_page`, `duplicate_across_pages` и
`duplicate_across_streams`; итоги запуска дедуплицируются по канонической
`vacancy_id` либо HH ID. Изменение позиции в выдаче не считается материальным
изменением вакансии. Новый заголовок, компания, публикация или канонический URL
консервативно ставят известный ID в ограниченную очередь подробностей. Текст страницы
списка не перезаписывает более сильное доказательство со страницы вакансии или
из жизненного цикла. Обычный ответ CLI содержит только настроенное число новых
и изменённых ID; полный список
доступен лишь с явным `record-hh-page --verbose`.

Персональные рекомендации — самостоятельный `source_kind` и отдельный шаг P1. Для
них используется `capturePersonalRecommendations`, собственная контрольная точка,
начальная глубина, граница новизны и максимальная глубина. Максимум страниц
сам по себе не доказывает завершение, если это явно не разрешено настройкой.
Видимый `search_session_id` проверяется по хешу; если источник действительно
его не показывает, адаптер фиксирует `not_exposed` и детерминированный
альтернативный отпечаток. Необъяснимо пустая или изменившаяся идентичность сессии
блокирует источник.

Артефакты снимков и манифестов сохраняются внутри выбранного `JOB_SEARCH_HOME`
как нормализованный gzip JSON с версией и SHA-256. Полный HTML авторизованной
страницы, cookie, токены и идентификаторы аккаунта по умолчанию не сохраняются.
История успешных контрольных точек и аудитов неизменяема; доказательства незавершённого запуска не
удаляются автоматически. После закрытия и установленного локальной политикой
срока хранения оператор может архивировать старые завершённые артефакты, но не
должен удалять единственное доказательство блокировки или неудачного аудита.
...
Схема v11 обновляет все поддерживаемые версии v1–v10 и по умолчанию создаёт
резервную копию исходной базы. Она сохраняет все таблицы P2 схемы v10 и
добавляет append-only попытки 502, audit-only решения по отклонённым снимкам,
полные новые сессии и точные решения восстановления. Схема v10 обновляет все
поддерживаемые версии v1–v9 и по умолчанию создаёт
резервную копию исходной базы. Она сохраняет доказательства и оркестрацию v9,
добавляет контрольные точки для каждого потока HH, неизменяемую историю
контрольных точек и аудитов, состояние получения, снимки страниц и элементов,
снимки вакансий, ограниченную очередь подробностей и события инкрементальной
безопасности. Частичный, заблокированный или сомнительный поток никогда не
продвигает успешный курсор; успешно завершённый поток может сохранить
собственную контрольную точку, даже если другой обязательный поток оставил
общий ежедневный запуск незавершённым.

Схема v9 обновляла все поддерживаемые версии v1–v8. Она сохраняет полную модель
...
Разрушающей обратной миграции нет. После обновления до v11 нельзя открывать базу
старой версией Engine: она не знает долговечный план запуска или инкрементальный
курсор и может нарушить координацию. Повторный `migrate-schema` для схемы v11
идемпотентен и не создаёт новую резервную копию.
...
Схема v9, сохранённая без ослабления в v10, хранит текущий снимок запуска в
`daily_runs`, неизменяемые ревизии
...
Манифест HH v2 обратно совместим с валидатором v1 и дополнительно содержит
режим получения, отпечатки запроса и конфигурации, версию адаптера, данные о
доступности ID сессии, хеш и доказательство стабильности каждой страницы,
доказательство расхождения счётчика, результаты программной сверки, причину
остановки, доказательство границы, сравнение `shadow`/`audit` и возможный курсор. Для `full`, `shadow` и `audit`
нужна исчерпывающая пагинация; для `delta` полный хвост заменяет строгое
доказательство границы известных результатов. `resume` сохраняет исходный режим
и продолжает с точной последней проверенной страницы.
